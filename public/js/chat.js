...
function chatClearText() {
    ...
    try {
        //msgHistory.set('msg'+indx, '');
        //msgHistory.set('stime'+indx, null);
    } catch (e) {
        // ...
    }
};
...
function chatRequestData(msg, channelTalk, complain, crc, complain_nick, stime, report) {
    ...
    entry_point_request('chat', 'send', data, function(response){
        if (response.status == DATA_OK ) {
            ...
            if (response.msg) {
                chatReceiveMessage(response.msg);
            }
            if (response.msg_id) {
                last_msg_id = response.msg_id;
            }
        } else if (response.error) {
            showError(response.error, response.crc)
        }
    });
    return true;
}
...
function chatToggleFightlog(f, fight_id) {
    ...
    if (chatFightlog){
        ...
        gebi('fightlog').style.width = "300px";
    }
    else{
...
function chatToggleIgnore(name) {
    if(IGNORED[name] !== undefined && IGNORED[name]) {
        //Вытащить из игнора
        $.ajax({
            'url': 'friend_list.php',
            'data': {
...
function chatReceiveMessage(msg) {
    ...
    if (parseInt(msg.wheel_msg) && parseInt(session.no_wheel_msg)) {
        return false;
    }
...
function chatFormatMessage(msg) {
    ...
    if (msg.user_nick) {
        var info_nick = "";
        if(CHAT.nick_info){
            info_nick = "<a href='/user_info.php?nick="+escape(msg.user_nick)+"' target='_blank'> <img src='/images/player_info.gif' align='absmiddle'></a>";
        }
        if (mymsg) html += '<b>';
        html += '<a style="cursor:pointer; text-decoration: none;" ' + ctlMnu(msg.user_nick, msg.user_level, msg.user_kind) + ' onclick="' + t_nickfnc + '(\'' + escape(msg.user_nick) + '\'); return false;" ' +
            'title="' + t_nickalt + '">' + escape(msg.user_nick) + "</a>"+info_nick+"";
        if (msg.to_user_nicks) {
            var t = [];
            for (var i in msg.to_user_nicks) {
                ...
                var to_info_nick = "";
                if(CHAT.nick_info){
                    to_info_nick = "<a href='/user_info.php?nick="+escape(v)+"' target='_blank'> <img src='/images/player_info.gif' align='absmiddle'></a>";
                }
                t.push('<a style="cursor: pointer; text-decoration: none;" ' + ctlMnu(v, msg.to_user_levels[i], msg.to_user_kinds[i]) + ' onclick="'+t_nickfnc+'(\''+addslashes(v)+'\'); return false;" title="'+t_nickalt+'">'+escape(v)+'</a>'+to_info_nick+'');
            }
            if (t.length > 0) {
                html += '&nbsp;&raquo;&nbsp;' + t.join(', ');
            }
    ...
    html += '</span>';
    if(msg.type == msg_type.broadcast && msg.user_id == 5){
        $.playSound("/images/audio/post.mp3");
    }
    if(msg.type == msg_type.system && msg.msg_text.indexOf("Saldırı başlatıldı")>= 0){
        if (top1.on_sound){
            $.playSound("/images/audio/battlestart.wav");
        }
    }
    if(msg.type == msg_type.def && mymsg>= 0){
        if (top1.on_sound){
            // $.playSound("/images/audio/prvmsgreceive.wav");
        }
    }

    if(msg.type == msg_type.system && msg.msg_text.indexOf("Savaş alanında katılımlarını onaylamak için")>= 0){
        if (top1.on_sound){
            $.playSound("/images/audio/instancestart.wav");
        }
    }
    if(msg.type == msg_type.def && msg.msg_text.indexOf("ramazan.gif")>= 0){
        $.playSound("/images/audio/davul.mp3");
    }
    if(msg.type == msg_type.system && msg.msg_text.indexOf("#FISEK#")>= 0){
        $.playSound("/images/audio/fisek.mp3");
        msg.msg_text = msg.msg_text.replace("#FISEK#", "")
    }
...
var msgHistory = {
    ...
    set: function(key, value) {
        if (!this.obj) return;
        this.obj.setValue(key, value);
    },
    append: function(key, value) {
        if (!this.obj) return;
        this.obj.appendValue(key, value);
    }
};
...
function initMsgHistory() {
    var obj = document.msgHistory_obj;

    if (obj && obj.getValue && obj.setValue) {
        debugLog('restoring message history');
        msgHistory.obj = obj;
        restoreChatMessages();
        debugLog('message history restored');
        return;
    }
    setTimeout(initMsgHistory, 300);
};
...
function chat_channel_settings_save(form, current_channel) {
    ...
    $.ajax({
        ...
        CHAT.nick_info = chat_data.chat_info;
        chat_update_channel_settings(CHAT.channel_settings[current_channel], current_channel);
        chat_update_text_size(CHAT.text_size);
    });
    return false;
...
function show_last_messages(){
    showMsg2('last_messages.php',350,1000);
}

var chat_change_time_zone = show_last_messages;
